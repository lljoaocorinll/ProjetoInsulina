...
lb_glicemia.configure(janela, font=('Arial', 15))
...
lb_horario.place(x=325, y=100)
ent_horario.place(x=360, y=130)
ent_horario.configure(height=5, width=35)
lb_glicemia.place(x=325, y=160)
ent_glicemia.place(x=360, y=190)
ent_glicemia.configure(height=5, width=35)
botao.place(x=325, y=300)
botao.configure(width=100, height=50)
lb_txt_resultados.place(x=275, y=400)
lb_resultados_anteriores.pack(expand=True, padx=1, pady=1)
lb_resultados_anteriores.configure(font=('Arial', 20))
frame.place(x=190, y=490)
frame.configure(width=350, height=350)
janela.mainloop()

''''''-------------------------------------------------------------------------------------------'''
        if 71 < glicemia <= 100:
            print('Pare a Infusão e comunique o Médico Plantonista !!!')
            glicinicial = glicemia
            vazaoatual = 0
            break
        if glicemia <= 71:
            print('Pare a Infusão, comunique o Médico Plantonista e administre 40ml de G50% !!!')
            glicinicial = glicemia
            vazaoatual = 0
            break
    elif glicemia >= glicinicial:
        if 141 < glicemia <= 220:
            print(f'Alterar vazão para {vazaoatual + 1}ml/h')
            glicinicial = glicemia
            vazaoatual = (vazaoatual + 1)
        if 220 < glicemia <= 260:
            print(f'Alterar vazão para {vazaoatual + 2}ml/h')
            glicinicial = glicemia
            vazaoatual = (vazaoatual + 2)
        if glicemia > 260:
            print(f'Alterar vazão para {vazaoatual + 3}ml/h')
            glicinicial = glicemia
            vazaoatual = (vazaoatual + 3)
        if 101 < glicemia <= 140:
            print(f'Alterar vazão para {int(vazaoatual / 2)}ml/h')
            glicinicial = glicemia
            vazaoatual = (vazaoatual / 2)
        if 71 < glicemia <= 100:
            print('Pare a Infusão e comunique o Médico Plantonista !!!')
